...
void getProceduralColorWithPosition(vec3 rayOrigin, vec3 rayDir, out vec3 fragColor, out vec3 rayPos)
{
...
float getProceduralFragmentWithPosition(inout ProceduralFragmentWithPosition frag) {
	...
	vec3 offset = vec3(0,-110,0);

	rayOrigin -= offset;

	vec3 fragColor = vec3(0);
	vec3 rayPos = vec3(0);
	getProceduralColorWithPosition(rayOrigin, rayDir, fragColor, rayPos);

	frag.position = rayPos+offset;
